...
function SavingsGoals() {
  ...
  const fetchMonthlySavings = async () => {
    try {
      ...
      console.log("Monthly savings API response:", data);

      if (Array.isArray(data)) {
        setMonthlySavings(data);
      } else if (Array.isArray(data.results)) {
        setMonthlySavings(data.results);
      } else {
        setMonthlySavings([]);
      }
    } catch (err) {
  ...
  const getMonthlyRequired = (goal) => {
    ...
    const monthsRemaining =
      (deadlineDate.getFullYear() - selectedDate.getFullYear()) * 12 +
  ...
  const selectedMonthData = (
    Array.isArray(monthlySavings) ? monthlySavings : []
  ).filter((item) => {
    if (!item || !item.month) {
      return false;
    }

    return String(item.month).slice(0, 7) === selectedMonth;
  });
  ...
  const saveMonthlyAmount = async (goal) => {
    ...
    const existingRecord = (
      ...
      (item) =>
        ...
        item.month &&
        String(item.month).slice(0, 7) === selectedMonth
    );
                        ...
                        {monthlyEditGoalId !== g.id && (
                          <button
                            ...
                            onClick={() => {
                              ...
                              setMonthlyEditGoalId(
                                g.id
                              );

                              setMonthlyEditAmount(
                                currentMonthlyRecord
                                  ? currentMonthlyRecord.saved_amount
                                  : ""
                              );
